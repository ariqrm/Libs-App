...
const book = (state = initialState, action) => {
  switch (action.type) {
    ...
    case 'TRANSACTION_BOOK_REJECTED':
      return {
        ...state,
        isLoading: false,
        isRejected: true,
      };
    case 'TRANSACTION_BOOK_FULFILLED':
      // var arr = state.bookList.splice(state.bookList.indexOf(state.bookDetail[0]), 1);
      var arr = state.bookList;
      arr.splice(arr.indexOf(state.bookDetail[0]), 1);
      arr;
      return {
        ...state,
        bookList: arr,
